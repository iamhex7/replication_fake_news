...
import delimited `"data/raw_data.csv"', clear
...
drop if current_page != "Results" | index_in_pages != 155
drop round_number
...
** two people started the study twice; their second observations are deleted
drop if code == "8xs61s5e" & substr(time_start, 12, 5) == "23:25"
drop if code == "ogdc5pj0" & substr(time_start, 12, 5) == "20:29"
...
gen quiz_score = ((quiz1 == "Tim Kaine") + (quiz2 == "Doug Jones") + (quiz3 == "Neil Gorsuch") + (quiz4 == "Emmanuel Macron"))
replace quiz_score = . if age == .
rename quiz_score quiz_score_temp 
bysort code: egen quiz_score = max(quiz_score_temp)
...
/* Treatments: 
	P = WTP elicited in round 12
	S = second-guess treatment
	Y = given prior of P(True) = 0.5
	N = not given prior
	R = in Round 14, told average Dem score and asked to predict average Rep score
	D = in Round 14, told average Rep score and asked to predict average Dem score
*/
gen treatment_p = (substr(treatment, 1, 1) == "P")
gen treatment_s = (substr(treatment, 1, 1) == "S")
gen treatment_y = (substr(treatment, 2, 1) == "Y")
gen treatment_n = (substr(treatment, 2, 1) == "N")
gen treatment_r = (substr(treatment, 3, 1) == "G")
gen treatment_d = (substr(treatment, 3, 1) == "D")

** overall scores (note: different for treatment P and S because of the WTP elicitation)
gen avg_overall = .

replace avg_overall = (avg_guess_points + avg_lower_bound_points + ///
avg_upper_bound_points + avg_news_points) / 4 ///
if substr(treatment, 1, 1) == "P"

replace avg_overall = (avg_guess_points + avg_lower_bound_points + ///
avg_upper_bound_points + avg_reguess_points + avg_news_points) / 5 ///
if substr(treatment, 1, 1) == "S"
...
/* 	
	...
	9				Random Number				Neutral				1-12
	10				Longitude					Neutral				1-12
	11				Latitude					Neutral				1-12
...
gen politicized_news = (topic_id <= 8 | topic_id == 14 | topic_id == 15)
gen neutral_news = (topic_id >= 9 & topic_id <= 11)
...
bysort code: egen wtp = max(wtp_cutoff)
rename wtp_cutoff wtp_topic
...
gen neutral_true = (true_news & neutral_news)
...
bysort code: egen prob_true_mean = mean(prob_true) if message_greater + message_less & politicized_news + neutral_news
...
export delimited "data/cleaned_data_withfailedcheck.csv", replace

												   
** out_of_range if subjects give answers that are out of a reasonable range
gen out_of_range_q = 0
replace out_of_range_q = out_of_range_q + 1 ///
	if topic_id == 1 & (your_lower_bound < 0 | your_upper_bound > 100 | (your_reguess != . & (your_reguess < 0 | your_reguess > 100)))
replace out_of_range_q = out_of_range_q + 1 ///
	if topic_id == 2 & (your_lower_bound < 0 | your_upper_bound > 100000 | (your_reguess != . & (your_reguess < 0 | your_reguess > 100000)))
replace out_of_range_q = out_of_range_q + 1 ///
	if topic_id == 3 & (your_lower_bound < 0 | your_upper_bound > 1000000 | (your_reguess != . & (your_reguess < 0 | your_reguess > 1000000)))
replace out_of_range_q = out_of_range_q + 1 ///
	if topic_id == 5 & (your_lower_bound < 0 | your_upper_bound > 100 | (your_reguess != . & (your_reguess < 0 | your_reguess > 100)))
replace out_of_range_q = out_of_range_q + 1 ///
	if topic_id == 6 & (your_lower_bound < 0 | your_upper_bound > 4 | (your_reguess != . & (your_reguess < 0 | your_reguess > 4)))
replace out_of_range_q = out_of_range_q + 1 ///
	if topic_id == 7 & (your_lower_bound < 0 | your_upper_bound > 100 | (your_reguess != . & (your_reguess < 0 | your_reguess > 100)))
replace out_of_range_q = out_of_range_q + 1 ///
	if topic_id == 8 & (your_lower_bound < 0 | your_upper_bound > 100 | (your_reguess != . & (your_reguess < 0 | your_reguess > 100)))
replace out_of_range_q = out_of_range_q + 1 ///
	if topic_id == 9 & (your_lower_bound < 0 | your_upper_bound > 100 | (your_reguess != . & (your_reguess < 0 | your_reguess > 100)))
replace out_of_range_q = out_of_range_q + 1 ///
	if topic_id == 10 & (your_lower_bound < 0 | your_upper_bound > 180 | (your_reguess != . & (your_reguess < 0 | your_reguess > 180)))
replace out_of_range_q = out_of_range_q + 1 ///
	if topic_id == 11 & (your_lower_bound < 0 | your_upper_bound > 90 | (your_reguess != . & (your_reguess < 0 | your_reguess > 90)))
replace out_of_range_q = out_of_range_q + 1 ///
	if topic_id == 12 & (your_lower_bound != 2018 | your_upper_bound != 2018 | prob_true != 1 | (your_reguess != . & (your_reguess != 2018)))
replace out_of_range_q = out_of_range_q + 1 ///
	if topic_id == 13 & (your_lower_bound < 0 | your_upper_bound > 100 | (your_reguess != . & (your_reguess < 0 | your_reguess > 100)))
replace out_of_range_q = out_of_range_q + 1 ///
	if topic_id == 14 & (your_lower_bound < 0 | your_upper_bound > 100 | (your_reguess != . & (your_reguess < 0 | your_reguess > 100)))
replace out_of_range_q = out_of_range_q + 1 ///
	if topic_id == 15 & (your_lower_bound < 0 | your_upper_bound > 100 | (your_reguess != . & (your_reguess < 0 | your_reguess > 100)))
	
** drop subjects who are out of range
bysort code: egen out_of_range = max(out_of_range_q)
